...
const page = ({ params }) => {
    ...
    const orderProductType = Number(searchParams.get("order-type"));
        ...
        if (session.accessToken) {
            ...
            try {
                ...
                if (response.code === 200) {
                    router.push(
                        `/thankyou?orderId=${response?.results?.order_id}`
                    );
                } else {
        ...
        const fetchOrderDetails = async () => {
            try {
                ...
                if (session?.accessToken && orderId) {
                    ...
                    const response = await getResaleOrderByOrderId(
                        session.accessToken,
                        orderId
                    );
                ...
                <div className="container">
                    <div className="row gy-5 gy-lg-0 gx-0 gx-lg-5">
                        <div className="col-lg-8">
                            ...
                            <ResaleShippingProduct cartProduct={cartProduct} />
                        </div>
